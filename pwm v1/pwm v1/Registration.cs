...
namespace pwm_v1
{
    public partial class Registration : Form
    {
        ...
        private void BtnRegister_Click(object sender, EventArgs e)
        {
            try
            {
                //store return value of Emailvalidation method
                int EvalidateResult;

                //Check the text boxes are not empty
                if (Txt_UserName.Text != "" && Txt_UserEmail.Text != "" && Txt_Password.Text != "" && Txt_Confirm_Password.Text != "")
                {
                    //create object of EmailValidation class
                    //call for Emailvalidation class
                    EmailValidation validation = new EmailValidation();
                    EvalidateResult = validation.Emailvalidation(Txt_UserEmail.Text);

                    if (EvalidateResult == 1)
                    {
                        try
                        {
